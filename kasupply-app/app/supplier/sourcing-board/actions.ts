...
export async function getSupplierDashboardData(): Promise<SupplierDashboardData> {
  ...
  const sourcingOpportunities = requestMatchesResult.map((row) => {
    ...
    const requestIsClosed =
      ["closed", "fulfilled", "cancelled"].includes(
        String(rfq?.status ?? "").toLowerCase(),
      ) || isPastDeadline(rfq?.deadline);
    const buyerName =
    ...
    return {
      ...
      statusLabel: requestIsClosed ? "Closed" : "Open",
  ...
  const notificationItems: Array<SupplierDashboardData["notifications"][number] | null> = [
  ...
  const notifications = notificationItems
    .filter((item): item is SupplierDashboardData["notifications"][number] => {
      return item !== null && Number.isFinite(item.sortTime);
    })
