...
import { acceptSourcingOffer, getSourcingRequestDetails } from "./actions";
...
export default async function SourcingRequestDetailPage({
  ...
  const data = await getSourcingRequestDetails(numericRfqId);
          ...
          <section className="rounded-2xl border border-[#edf1f7] bg-white p-6 shadow-[0_10px_28px_rgba(15,23,42,0.04)]">
            <div>
              ...
              <p className="mt-1 text-sm text-[#8b95a5]">
                These suppliers engaged with your sourcing request and submitted negotiation offers or quotations.
              </p>
            </div>
        ...
        <div className="space-y-6">
          <section className="rounded-2xl border border-[#edf1f7] bg-white p-6 shadow-[0_10px_28px_rgba(15,23,42,0.04)]">
            ...
            <div className="mt-4 space-y-3 text-sm text-[#4a5b75]">
              ...
              <p>
                <span className="font-medium text-[#223654]">Total ranked suppliers:</span>{" "}
                {data.requestMatches.totalCount}
              </p>
              <p>
                <span className="font-medium text-[#223654]">Preferred delivery:</span>{" "}
            ...
            <div className="mt-4 space-y-3">
              {data.requestMatches.suppliers.length === 0 ? (
                <p className="text-sm text-[#8b95a5]">
                  No suppliers reached the current matching threshold.
                </p>
              ) : (
                data.requestMatches.suppliers.map((supplier) => (
                  <div
                    key={supplier.matchId}
                  ...
                  >
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="rounded-full border border-[#d7dee8] bg-white px-2 py-1 text-xs text-[#4a5b75]">
                        {supplier.matchScore == null ? "-" : `${supplier.matchScore}%`}
                      </span>
                    </div>
                    <p className="mt-2 text-sm text-[#8b95a5]">
                      {supplier.matchReason || "No match reason available."}
                    </p>
                  </div>
                ))
              )}
